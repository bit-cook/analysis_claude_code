...
通知バスは `queue.Queue` で実装されている。メインエージェントループは各API呼び出しの前に**ドレインとインジェクト**のサイクルを実行する:
...
# 3. インジェクト: 最後のuserメッセージに追加（またはメッセージを新規作成）
...
モデルは通知を会話コンテキスト内の構造化XMLブロックとして認識し、`TaskOutput` で完全な出力を取得するかサマリーで作業を継続するかを判断する。

## 通知フォーマット

バックグラウンドタスク完了時、通知がメインエージェントの次のターンに自動注入:
...
`summary`フィールドにはタスク出力の先頭500文字が含まれる。モデルが完全な結果を取得すべきかどうかを判断するのに十分な量。
...
メインエージェント:
  ...
  5. <task-notification> b5e8f1 completed   (ESLintが待機中に完了)
...
[<< v6](./v6-タスクシステム.md) | [READMEに戻る](../README_ja.md) | [v8 >>](./v8-チームメイト機構.md)
